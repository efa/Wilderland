...
/**                                                         **/
/** Adapted by CH, 2010. Fixed warnings by V.Messina 2022   **/
/*************************************************************/
...
/*************************************************************/
//extern byte mem[];
//INLINE byte RdZ80(word A)        { return(mem[A]); }
//INLINE void WrZ80(word A,byte V) { mem[A]=V; }


/*** INSTRUCTION-MACROS **************************************/
/**                                                         **/
/**  S Z - H - P/V N C   in F-Register                      **/
/**                                                         **/
/**  C if the operation produced a carry from the MSB of    **/
/**    the operand or result.                               **/
/**  S Set if the MSB of the result is one.                 **/
/**  P/V logical operations affect parity (0=odd,1=even)    **/
/**      arithmetic operations affect overflow.             **/
/**  H,N are used only in conjuction with the DAA instr.    **/
/**                                                         **/
/**  ? ... flag is set according to result                  **/
/**  x ... flag is unchanged by operation                   **/
/**  0 ... flag is reset by operation                       **/
/**  1 ... flag is set by operation                         **/
/**  % ... flag is "don't care" (might not be true for all) **/
/*************************************************************/
...
#include "CodesCB.h"
      default:
         if(R->TrapBadOps)
            printf(
               "[Z80 %zX] Unrecognized instruction: CB %02X at PC=%04X\n",
               (size_t)(R->User),RdZ80(R->PC.W-1),R->PC.W-2
            );
   }
}
...
#include "CodesXCB.h"
      default:
         if(R->TrapBadOps)
            printf(
               "[Z80 %zX] Unrecognized instruction: DD CB %02X %02X at PC=%04X\n",
               (size_t)(R->User),RdZ80(R->PC.W-2),RdZ80(R->PC.W-1),R->PC.W-4
            );
   }
#undef XX
}
...
#include "CodesXCB.h"
      default:
         if(R->TrapBadOps)
            printf(
               "[Z80 %zX] Unrecognized instruction: FD CB %02X %02X at PC=%04X\n",
               (size_t)R->User,RdZ80(R->PC.W-2),RdZ80(R->PC.W-1),R->PC.W-4
            );
   }
#undef XX
...
#include "CodesED.h"
      ...
      default:
         if(R->TrapBadOps)
            printf(
               "[Z80 %zX] Unrecognized instruction: ED %02X at PC=%04X\n",
               (size_t)R->User,RdZ80(R->PC.W-1),R->PC.W-2
            );
   }
}
...
#include "CodesXX.h"
      ...
      default:
         if(R->TrapBadOps)
            printf(
               "[Z80 %zX] Unrecognized instruction: DD %02X at PC=%04X\n",
               (size_t)R->User,RdZ80(R->PC.W-1),R->PC.W-2
            );
   }
#undef XX
